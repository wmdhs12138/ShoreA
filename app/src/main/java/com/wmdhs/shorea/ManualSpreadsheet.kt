...
internal data class SpreadsheetImport(
    ...
    val compoundCount: Int get() = manual.compounds.size
    val entryCount: Int get() = manual.inspectionEntries.size
    val partCount: Int get() = manual.totalPartAssociationCount
}

internal fun decodeManualSpreadsheet(
    input: InputStream,
): Result<SpreadsheetImport> = runCatching {
    ...
    val sheet = entries.entries
        .firstOrNull { it.key.startsWith("xl/worksheets/sheet") }
        ?.value
    ...
    require(rows.size >= 2 && cell(rows[0], 0).contains("胶料号")) {
        "表格格式不正确：第一列应为胶料号"
    }
    ...
    data class CompoundBuilder(
        ...
        var time: String = "",
        val entries: LinkedHashMap<String, EntryBuilder> = linkedMapOf(),
    )

    val compoundBuilders = linkedMapOf<String, CompoundBuilder>()
    var sourceRows = 0
    var skippedRows = 0
    rows.drop(2).forEach { row ->
        ...
        val compoundKey = code.uppercase(Locale.ROOT)
        val compound = compoundBuilders.getOrPut(compoundKey) {
            CompoundBuilder(code = code)
        }
        compound.temperature = compound.temperature.ifBlank {
            cleanOptional(cell(row, 1))
        }
        compound.time = compound.time.ifBlank {
            cleanOptional(cell(row, 2))
        }
        val standardKey = standard.uppercase(Locale.ROOT)
        val inspectionEntry = compound.entries.getOrPut(standardKey) {
            EntryBuilder(standardNumber = standard)
        }
        inspectionEntry.parts += part
        inspectionEntry.category = inspectionEntry.category.ifBlank {
            cleanOptional(cell(row, 5))
        }
        inspectionEntry.color = inspectionEntry.color.ifBlank {
            cleanOptional(cell(row, 6))
        }
        inspectionEntry.testPiece = inspectionEntry.testPiece.ifBlank {
            cleanOptional(cell(row, 7))
        }
        inspectionEntry.block = inspectionEntry.block.ifBlank {
            cleanOptional(cell(row, 8))
        }
        inspectionEntry.product = inspectionEntry.product.ifBlank {
            cleanOptional(cell(row, 9))
        }
        inspectionEntry.tensile = inspectionEntry.tensile.ifBlank {
            cleanOptional(cell(row, 10))
        }
        inspectionEntry.elongation = inspectionEntry.elongation.ifBlank {
            cleanOptional(cell(row, 11))
        }
    }
    require(compoundBuilders.isNotEmpty()) { "表格中没有可导入的资料" }

    val compoundIds = SpreadsheetIdAllocator()
    val entryIds = SpreadsheetIdAllocator()
    val compounds = mutableListOf<RubberCompound>()
    val inspectionEntries = mutableListOf<InspectionEntry>()
    compoundBuilders.values.forEach { source ->
        val compoundId = compoundIds.allocate()
        compounds += RubberCompound(
            id = compoundId,
        ...
        source.entries.values.forEach { entry ->
            inspectionEntries += InspectionEntry(
                ...
                partNumbers = normalizePartNumbers(entry.parts),
                hardness = HardnessSet(
                    testPieceHardness = entry.testPiece,
                    blockHardness = entry.block,
                    productHardness = entry.product,
                ).normalized(),
                productCategory = entry.category,
                color = entry.color,
                tensileStrength = entry.tensile,
                elongation = entry.elongation,
            )
        }
    }
    val manual = validateManual(
        HardnessManual(
            compounds = compounds,
            inspectionEntries = inspectionEntries,
        ),
    ).getOrThrow()
    SpreadsheetImport(manual, sourceRows, skippedRows)
}

internal fun encodeManualSpreadsheet(
    ...
    ZipOutputStream(output.buffered()).use { zip ->
        ...
        zip.textEntry(
            ...
            worksheetXml(manual, compoundsById),
        )
    }
}
...
private class SpreadsheetIdAllocator {
    private val ids = mutableSetOf<Long>()
    private var candidate = nextEntityId(ids)

    fun allocate(): Long {
        while (!ids.add(candidate)) {
            candidate = nextEntityId(ids)
        }
...
private fun columnIndex(reference: String): Int {
    var result = 0
    reference.takeWhile(Char::isLetter).forEach {
        result = result * 26 + (it.uppercaseChar() - 'A' + 1)
    }
    return (result - 1).coerceAtLeast(0)
}

private fun cell(row: Map<Int, String>, index: Int): String =
    row[index].orEmpty().trim()

private fun cleanRequired(value: String): String = value.trim()

private fun cleanOptional(value: String): String = value.trim()
    .takeUnless { it in setOf("/", "-", "—") }
    .orEmpty()
...
private fun worksheetXml(
    manual: HardnessManual,
    compoundsById: Map<Long, RubberCompound>,
): String {
    val rows = mutableListOf<List<String>>()
    rows += listOf(
        "胶料号", "硫化条件试片", "", "标准号", "部品号", "产品类别",
        "颜色", "性能", "", "", "", "",
    )
    rows += listOf(
        "", "温度/℃", "时间/min", "", "", "", "",
        "硬度片/A", "硬度块/A", "硬度产品/A", "拉伸强度/Mpa", "伸长率/%",
    )
    manual.inspectionEntries.forEach { entry ->
        val compound = compoundsById[entry.compoundId]
            ?: error("导出时发现孤立检测标准 ${entry.id}")
        normalizePartNumbers(entry.partNumbers).forEach { part ->
            rows += listOf(
                ...
                entry.color,
                entry.hardness.testPieceHardness,
                entry.hardness.blockHardness,
                entry.hardness.productHardness,
                entry.tensileStrength,
                entry.elongation,
            )
        }
    }
    val body = rows.mapIndexed { rowIndex, values ->
        val cells = values.mapIndexed { column, value ->
            val reference = columnName(column) + (rowIndex + 1)
